...
public class LeetCode_212 {
    ...
    private void dfs(char[][] board, int row, int col, StringBuilder stringBuilder, Trie trie) {
        if (row < 0 || col < 0 || row >= board.length || col >= board[row].length || memo[row][col]) return;
        stringBuilder = stringBuilder.append(board[row][col]);
        memo[row][col] = true;
        ...
        if (trie.startsWith(stringBuilder.toString())) {
            ...
            for (int i = 0; i < dx.length; i++) {
                dfs(board, row + dx[i], col + dy[i], stringBuilder, trie);
            }
        }
